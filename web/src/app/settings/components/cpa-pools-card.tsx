      ...
      <CardContent className="space-y-6 p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold tracking-tight">{t("cpa.header.title")}</h2>
              <p className="text-sm text-stone-500">{t("cpa.header.description")}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {pools.length > 0 ? <Badge className="rounded-md px-2.5 py-1">{t("cpa.header.connectionCount", { count: pools.length })}</Badge> : null}
            <Button className="h-9 rounded-xl bg-stone-950 px-4 text-white hover:bg-stone-800" onClick={openAddDialog}>
              <Plus className="size-4" />
              {t("cpa.actions.addConnection")}
            </Button>
          </div>
        </div>
        ...
        {isLoadingPools ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-stone-50 px-6 py-10 text-center">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium text-stone-600">{t("cpa.empty.title")}</p>
              <p className="text-sm text-stone-400">{t("cpa.empty.description")}</p>
            </div>
          </div>
        ) : (
                ...
                <div key={pool.id} className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3">
                  ...
                  {importJob ? (
                    <div className="space-y-2 rounded-xl bg-stone-50 px-3 py-3">
                      <div className="text-xs font-medium tracking-[0.16em] text-stone-400 uppercase">{t("cpa.item.importJobLabel")}</div>
                      <div className="rounded-lg border border-stone-200 bg-white px-3 py-3">
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <div className="text-sm font-medium text-stone-700">
                              {t("cpa.item.importStatus", { status: importJob.status, completed: importJob.completed, total: importJob.total })}
                            </div>
                            <div className="truncate text-xs text-stone-400">
                              {t("cpa.item.jobMeta", { jobId: importJob.job_id.slice(0, 8), createdAt: importJob.created_at })}
                            </div>
                          </div>
                          <Badge
                            variant={
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-xs text-stone-500">
                          <span>{t("cpa.item.added", { count: importJob.added })}</span>
                          <span>{t("cpa.item.skipped", { count: importJob.skipped })}</span>
                          <span>{t("cpa.item.refreshed", { count: importJob.refreshed })}</span>
                          <span>{t("cpa.item.failed", { count: importJob.failed })}</span>
                        </div>
                      </div>
                    </div>
                  ) : null}
        ...
        <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
          <p className="font-medium text-stone-600">{t("cpa.help.title")}</p>
          <ul className="mt-1 list-inside list-disc space-y-0.5">
            <li>{t("cpa.help.step1")}</li>
            <li>{t("cpa.help.step2")}</li>
            <li>{t("cpa.help.step3")}</li>
            <li>{t("cpa.help.step4")}</li>
          </ul>
        </div>
      </CardContent>
    </Card>
