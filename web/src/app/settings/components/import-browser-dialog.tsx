    ...
    <Dialog open={browserOpen} onOpenChange={setBrowserOpen}>
      <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
        <DialogHeader className="gap-2">
          <DialogTitle>{t("importBrowser.title")}</DialogTitle>
          <DialogDescription className="text-sm leading-6">
            {browserPool ? t("importBrowser.descriptionFrom", { name: browserPool.name || browserPool.base_url }) : t("importBrowser.descriptionDefault")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative min-w-[260px]">
            ...
            <Input
              ...
              placeholder={t("importBrowser.searchPlaceholder")}
          ...
          <div className="flex items-center gap-2">
              ...
              <SelectContent>
                {PAGE_SIZE_OPTIONS.map((item) => (
                  <SelectItem key={item} value={item}>
                    {t("importBrowser.pageSizeOption", { size: item })}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            >
              {allFilteredSelected ? t("importBrowser.deselectAll") : t("importBrowser.selectAllFiltered")}
            </Button>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-stone-200">
          <div className="flex items-center justify-between border-b border-stone-100 px-4 py-3 text-sm text-stone-500">
            <div className="flex items-center gap-3">
              ...
              <span>{t("importBrowser.filteredCount", { total: filteredFiles.length })}</span>
            </div>
            <span>{t("importBrowser.selectedCount", { total: selectedNames.length })}</span>
          </div>
          <div className="max-h-[420px] overflow-auto">
            {pagedFiles.length === 0 ? (
              <div className="flex items-center justify-center py-12 text-sm text-stone-400">{t("importBrowser.emptyAccounts")}</div>
            ) : (
        ...
        <div className="flex items-center justify-between text-sm text-stone-500">
          <span>
            {t("importBrowser.pageRange", {
              from: filteredFiles.length === 0 ? 0 : (safeFilePage - 1) * currentPageSize + 1,
              to: Math.min(safeFilePage * currentPageSize, filteredFiles.length),
        ...
        <DialogFooter className="pt-2">
          ...
          >
            ...
            {t("importBrowser.importSelected")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
