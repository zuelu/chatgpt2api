...
export function ImageSourcePicker({
  ...
  const historyItems = useMemo(() => {
    const collected: Array<{ key: string; src: string; b64?: string; url?: string; createdAt: string }> = [];
      ...
      for (const turn of conversation.turns) {
        ...
        for (const image of turn.images) {
          ...
          if (image.b64_json) {
            collected.push({
              key: image.id,
              src: b64ToBlobUrl(image.b64_json),
              b64: image.b64_json,
  ...
  const loadGallery = useCallback(async () => {
    ...
    try {
      ...
      setGalleryError(error instanceof Error ? error.message : "加载图库失败");
    } finally {
  ...
  const pickHistoryItem = async (item: { b64?: string; url?: string; key: string }) => {
    ...
    try {
      const dataUrl = item.b64 ? `data:image/png;base64,${item.b64}` : await remoteUrlToDataUrl(item.url || "");
      onPick({ name: `history-${item.key}.png`, type: "image/png", dataUrl });
      onOpenChange(false);
    } catch (error) {
      setGalleryError(error instanceof Error ? error.message : "读取图片失败");
    } finally {
      setLoadingKey("");
    }
  };
  ...
  const pickGalleryItem = async (item: ManagedImage) => {
    ...
    setLoadingKey(key);
    try {
      const dataUrl = await remoteUrlToDataUrl(item.url);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[min(80dvh,720px)] w-[94vw] max-w-[720px] flex-col overflow-hidden rounded-[28px] p-0">
        <DialogHeader className="px-6 pt-6 pb-3">
          <DialogTitle className="text-lg font-bold tracking-tight">选择要编辑的图片</DialogTitle>
        </DialogHeader>
        ...
        <Tabs value={tab} onValueChange={setTab} className="flex min-h-0 flex-1 flex-col gap-0">
          ...
          {galleryError ? (
            <div className="mx-6 mb-2 rounded-xl bg-rose-50 px-3 py-2 text-xs text-rose-700">{galleryError}</div>
          ) : null}

          <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-6">
            <TabsContent value="history" className="mt-0">
              {historyItems.length === 0 ? (
                <EmptyHint text="还没有生成过图片，先在创作里生成几张，或直接上传本地图片" />
              ) : (
