...
import { Eraser, Lasso, MessageSquarePlus, Redo2, Send, Undo2, X } from "lucide-react";
...
function buildTools(t: TFunction): Array<{ id: StudioTool; label: string; hint: string }> {
  return [
    { id: "comment", label: t("editor.tools.comment.label"), hint: t("editor.tools.comment.hint") },
    { id: "erase", label: t("editor.tools.erase.label"), hint: t("editor.tools.erase.hint") },
    { id: "lasso", label: t("editor.tools.lasso.label"), hint: t("editor.tools.lasso.hint") },
  ];
}
...
export function ImageEditStudio({
  ...
  const [baseMaskDataUrl, setBaseMaskDataUrl] = useState<string | null>(null);
  const [baseMaskImage, setBaseMaskImage] = useState<HTMLImageElement | null>(null);
  ...
  useEffect(() => {
    ...
    setBrushStrokes([]);
    setRedoStack([]);
    setBaseMaskDataUrl(initialSelection?.maskDataUrl ?? null);
  ...
  useEffect(() => {
    ...
    image.src = baseMaskDataUrl;
    return () => {
      cancelled = true;
    };
  }, [open, baseMaskDataUrl]);
  ...
  const buildMaskDataUrl = useCallback(
    (lasso: StudioPoint[][], brush: StudioPoint[][]): string | null => {
      ...
      const hasNewSelection = usableLasso.length > 0 || usableBrush.length > 0;
      if (width <= 0 || height <= 0) {
      ...
      if (baseMaskImage) {
        ctx.drawImage(baseMaskImage, 0, 0, width, height);
      } else {
    ...
    [baseMaskDataUrl, baseMaskImage, brushSize],
  );
  ...
  const paint = useCallback(() => {
    ...
    const hasSelection = Boolean(baseMaskDataUrl) || allLasso.length > 0 || allBrush.length > 0;

    if (hasSelection) {
      if (baseMaskImage) {
        ctx.globalAlpha = 0.5;
        ctx.drawImage(baseMaskImage, 0, 0, rect.width, rect.height);
        ctx.globalAlpha = 1;
      } else {
        // 未选中的区域压暗
  ...
  }, [activeTool, baseMaskDataUrl, baseMaskImage, lassoPaths, brushStrokes, brushSize]);
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    if (!imageUrl || busy || (baseMaskDataUrl && !baseMaskImage)) {
      return;
    }
  ...
  const hasSelection = Boolean(baseMaskDataUrl) || lassoPaths.length > 0 || brushStrokes.length > 0;
  const canUndo = lassoPaths.length > 0 || brushStrokes.length > 0;
  const canSubmit = Boolean(prompt.trim()) && !busy;
  const tools = useMemo(() => buildTools(t), [t]);
  const activeHint = useMemo(() => tools.find((tool) => tool.id === activeTool)?.hint ?? "", [activeTool, tools]);
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-white dark:bg-stone-950">
      ...
      <header className="relative flex shrink-0 items-center gap-3 px-4 py-3">
        <button
          ...
          aria-label={t("editor.actions.close")}
        >
          <X className="size-5" />
        </button>
      ...
      {/* 工具条 / 提示条 */}
      <div className="pointer-events-none absolute inset-x-0 top-14 z-20 overflow-x-auto px-2 pb-2">
        <div className="pointer-events-auto mx-auto flex w-max items-center gap-1 rounded-full border border-stone-200/80 bg-white/95 p-1 shadow-[0_18px_50px_-24px_rgba(15,23,42,0.45)] backdrop-blur dark:border-white/10 dark:bg-stone-900/95">
          {activeTool ? (
            <>
              ...
              {activeTool === "erase" ? (
                <>
                  ...
                  <span className="hidden text-[11px] text-stone-500 dark:text-stone-400 sm:inline">{t("editor.brush")}</span>
                  <input
                    type="range"
                    min={BRUSH_MIN}
                    max={BRUSH_MAX}
                    ...
                    aria-label={t("editor.actions.brushSize")}
              ...
              <button
                ...
                aria-label={t("editor.actions.exitTool")}
              >
                <X className="size-4" />
              </button>
            </>
          ...
          {annotations.map((annotation, index) => (
            ...
            >
              <button
                ...
                title={t("editor.annotation.removeTitle", { text: annotation.text })}
            ...
            >
              <div className="flex items-center gap-1 rounded-2xl border border-stone-200 bg-white p-1.5 shadow-lg dark:border-white/10 dark:bg-stone-900">
                <input
                  ...
                  placeholder={t("editor.annotation.placeholder")}
        ...
        <div className="flex w-full max-w-[820px] items-center gap-2 rounded-[26px] border border-stone-200 bg-white px-3 py-2 shadow-[0_10px_40px_-28px_rgba(15,23,42,0.45)] dark:border-white/10 dark:bg-stone-900">
          ...
          {annotations.length > 0 ? (
            <span className="shrink-0 rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-medium text-blue-700">
              {t("editor.annotation.count", { count: annotations.length })}
            </span>
          ) : null}
          <button
            type="button"
            ...
            aria-label={t("editor.actions.submit")}
          >
