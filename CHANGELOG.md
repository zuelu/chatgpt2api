...
+ [修复] 图片生成的内容政策关键词补充线上观察到的英文拒绝表述，匹配前统一弯引号，避免上游英文拒绝时轮询到超时。
+ [优化] 图片轮询超时、SSE 硬上限中断等失败写入调用日志时补记 conversation_id，网页生图任务的失败日志同样补记；SSE 结束日志增加 blocked 标记和文本预览。
+ [调整] 生图轮询超时与 SSE 流中断的错误文案改为标明阶段和已知事实，去掉配置项提示和原因猜测；设置页对应配置项改名为“生图阶段超时”并说明两阶段各自计时。
